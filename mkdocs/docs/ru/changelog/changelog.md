...
- Исправлено генерация `Form` генератором OpenAPI для аргументов `byte[]`
- Исправлено улучшение родительской модели `discriminator` генератором OpenAPI
- Исправлено `Validation` генератором OpenAPI для внутренних моделей
- Исправлено nullable элементы `Kotlin` генератором OpenAPI
- Исправлена проверка HttpServer `Undertow` для включенных `Virtual Threads`
- Улучшены некоторые сообщения об исключениях, ведение журнала
