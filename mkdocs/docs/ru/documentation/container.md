...
Компоненты внедряются только если являются [самостоятельным компонентом](#root-component), либо если требуются в других компонентах как зависимости.

Компоненты неудовлетворяющие этим требованиям не попадают в контейнер зависимостей.

#### Авто фабрика { #auto-factory }
...
#### Основная фабрика { #basic-factory }
...
#### Внешняя модуль фабрика { #external-module-factory }
...
то Kora может попробовать самостоятельно создать компонент если он удовлетворяет требованиям аналогичным [авто фабрики](#components):
...
### Переопределение компонент { #component-override }
...
### Список компонент { #list-of-components }
...
* Осуществляет [штатное завершение (Graceful Shutdown)](#component-lifecycle) при получении сигнала SIGTERM
...
### Жизненный цикл компонент { #component-lifecycle }
...
#### Обновление компонент { #updating-components }
...
### Инспекция компонент { #component-inspection }
