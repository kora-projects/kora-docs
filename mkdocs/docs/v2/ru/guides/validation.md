...
После компиляции сгенерированный AOP-прокси показывает, как `@Valid` делегирует работу в сгенерированный валидатор `UserRequest` до вызова метода контроллера:
...
После компиляции сгенерированный прокси показывает, как ограничения path-параметра становятся обычными вызовами валидаторов:
...
После компиляции сгенерированный прокси показывает, что необязательные query-параметры валидируются только когда присутствуют:
...
класс. Ваш код по-прежнему выглядит простым, но сгенерированный прокси выполняет проверки до того, как вызов попадет в тело метода.
...
- Изучите, как прокси валидирует аргументы перед делегированием в исходный метод контроллера.
