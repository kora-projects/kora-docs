...
- Добавлено `ConsumerRecordWrapper#unwrap` для доступа к исходному событию потребителя
...
- Исправлено `NPE` в `HttpServerResponseException.of` с пустым сообщением в брошенной ошибке
...
- Исправлена ошибка в OpenAPI генераторе, приводящая к `StackOverFlow` в цикле проверки иерархии моделей для `Validation`
