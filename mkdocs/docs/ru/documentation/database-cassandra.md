...
Общие правила для сущностей, `@Repository`, `@Query`, макросов, пакетных запросов и аннотаций `@Table`, `@Column`, `@Id`, `@Embedded` описаны в разделе [общих правил работы с базами данных](database-common.md).
...
Сущности описываются с помощью [общих аннотаций баз данных](database-common.md) и помечаются `@EntityCassandra`,
чтобы `Kora` сгенерировала отображатель сущности на этапе компиляции (см. [Сущность](#entity)):
...
Можно переопределить отображение различных частей [сущности](database-common.md) и параметров запроса — `Kora` предоставляет для этого специальные интерфейсы.
...
Тип `@UDT` отображается как любая другая сущность, поэтому охватывающая сущность помечается `@EntityCassandra`.
...
сущность и репозиторий выглядят так:
...
Если тип `UDT` используется не через охватывающую сущность, а как самостоятельный тип Cassandra, генерацию отображателя можно включить явно с помощью `@EntityCassandra`.
...
Cassandra драйвер использует общий контракт телеметрии для логирования, метрик и трассировки запросов.
...
Выполняемый запрос описывается `QueryContext(queryId, cql, operation)`, где `queryId` — стабильный идентификатор запроса,
`cql` — итоговый текст запроса, а `operation` по умолчанию равно `db_query`.
...
Метрики и трассировка описаны в разделе [Справочник метрик](metrics.md#cassandra).
