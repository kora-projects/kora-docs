...
===! ":material-code-json: `Hocon`"
    ...
    9. Kafka [потребитель](kafka.md#configuration) (Указывается для конкретного потребителя)
    10. Kafka [продюсер](kafka.md#manual-override) (Указывается для конкретного продюсера)
