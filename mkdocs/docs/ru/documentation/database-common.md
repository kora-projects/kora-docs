...
В этом разделе описана общая модель для `JDBC`, `Cassandra`, `R2DBC` и `Vertx`: сущности, репозитории, параметры запросов, пакетные запросы, количество затронутых строк и макросы.
...
Сущность — это представление данных из базы данных в виде класса с полями.

Сущности, используемые как возвращаемое значение, должны содержать единственный публичный
конструктор. Это может быть либо конструктор по умолчанию, либо конструктор с параметрами.
Если Kora находит конструктор с параметрами, объект сущности создаётся на его основе.
...
Вы можете указать, к какой таблице относится сущность — это понадобится, если вы используете [макросы](#macros) при построении запросов.
...
Поскольку все манипуляции с данными выполняются путём преобразования сущности в запрос драйвера,
нет необходимости выделять внутри сущности специальный первичный ключ для работы с ней.
...
Пример таблицы для такой сущности выглядел бы так:
...
пометив сам **метод** репозитория аннотацией `@Id` (аннотация применима как к полю сущности, так и к методу).
...
По умолчанию имена полей сущности при получении результата преобразуются в [`snake_lower_case`](https://www.freecodecamp.org/news/snake-case-vs-camel-case-vs-pascal-case-vs-kebab-case-whats-the-difference/).

Если вы хотите настроить отображение конкретных полей из базы данных на сущность, можно использовать аннотацию `@Column`:
...
Если вы хотите использовать стратегию именования для всей сущности, предлагается создать реализацию `NameConverter`, а затем использовать её в аннотации `@NamingStrategy`.
...
=== ":simple-kotlin: `Kotlin`"
    ...
    По умолчанию все поля, объявленные в сущности и не использующие синтаксис [Kotlin Nullability](https://kotlinlang.org/docs/null-safety.html), считаются **обязательными** (*NotNull*).
...
===! ":fontawesome-brands-java: `Java`"
    ...
    Если поле сущности необязательное, то есть может отсутствовать,
...
Если вы хотите использовать вложенные поля, то есть преобразовать поля сущности в отдельные классы, можно использовать аннотацию `@Embedded`.
...
Тогда репозиторий для такой сущности выглядел бы так:
...
Если параметр является сущностью или `DTO`, к его полям можно обращаться через точечную нотацию: `:entity.id`, `:entity.name`, `:filter.status`.
...
Её можно разместить на поле сущности, параметре метода или методе репозитория:

- на поле сущности — чтобы настроить чтение или запись конкретного столбца;
...
Отображатель параметра применяется к параметру запроса, отображатель столбца — к полю сущности, а отображатель результата или строки — к методу репозитория.
...
Самой утомительной частью написания SQL-запросов может быть перечисление и поддержание в актуальном состоянии столбцов и полей сущности.

Чтобы решить эту проблему, используйте специальные макросы внутри `SQL`-запроса в аннотации `@Query`.
Эти конструкции оперируют целевой [сущностью](#entity), разворачивают её в конкретные `SQL`-конструкции и упрощают расширение `SQL`-запросов.
...
4. После этого указывается команда макроса, которая сообщает, в какую SQL-конструкцию развернуть сущность
...
- `table` — разворачивает значение сущности из [аннотации](#table) `@Table`, либо, если она отсутствует, преобразует имя сущности в [`snake_lower_case`](https://www.freecodecamp.org/news/snake-case-vs-camel-case-vs-pascal-case-vs-kebab-case-whats-the-difference/)
- `selects` — создаёт конструкцию перечисления столбцов сущности для запроса `SELECT`
- `inserts` — создаёт конструкцию перечисления таблицы, столбцов и соответствующих полей сущности для запроса `INSERT`
- `updates` — создаёт конструкцию перечисления столбцов и соответствующих полей сущности для запроса `UPDATE`
- `where` — создаёт конструкцию перечисления столбцов со значением из сущности для части `WHERE` запроса
...
1. `=` — в разворачивании команды будут участвовать только поля сущности, имена которых указаны после символа
2. `-=` — в разворачивании команды будут участвовать все поля сущности, кроме указанных после символа
...
чтобы сразу обратиться к идентификатору сущности, помеченному [аннотацией](#identifier) `@Id`.
...
Пример полного репозитория со всеми основными методами для работы с сущностью для [Postgres SQL](https://postgrespro.com/docs/postgresql):
...
Пример репозитория с [составным идентификатором](#composite) и основными методами для работы с сущностью,
...
Телеметрия отдельного запроса строится фабрикой `DataBaseTelemetryFactory`, которая создаёт `DataBaseTelemetry` из
[конфигурации телеметрии](database-jdbc.md#configuration) драйвера и метаданных соединения
(имя пула, тип драйвера, тип базы данных и пользователь).
...
`DataBaseModule` (транзитивная часть модуля каждого драйвера, например `JdbcDatabaseModule`) предоставляет фабрики
`DefaultDataBaseTelemetryFactory` и `DataBaseLoggerFactory` как [компоненты по умолчанию](container.md#component-override),
поэтому фабрики метрик и трассировки из модулей [метрик](metrics.md) и [трассировки](tracing.md) автоматически подхватываются при их наличии.
Чтобы полностью настроить телеметрию, предоставьте собственную `DataBaseTelemetryFactory` в [графе приложения](container.md),
