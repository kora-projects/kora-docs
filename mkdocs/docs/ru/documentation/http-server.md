...
Он построен на асинхронных и неблокирующих I/O-операциях с использованием NIO, 
...
===! ":material-code-json: `Hocon`"
    ...
    3.  Порт внутреннего HTTP-сервера (для межсервисного взаимодействия)
    4.  Путь для получения [метрик](metrics.md) на служебном сервере
    5.  Путь для получения статуса [проб готовности](probes.md) на служебном сервере
    6.  Путь для получения статуса [проб жизнеспособности](probes.md) на служебном сервере
    7.  Игнорировать ли слэш в окончании пути, если включен то `/my/path` и `/my/path/` будут интерпритироваться одинакого, по умолчанию выключен
    8.  Количество потоков сетевых обработчиков, по умолчанию равен кол-во ядер процессора либо минимум `2`
    9.  Количество потоков обработчиков запросов, по умолчанию равен кол-во ядер процессора умноженных на 2 либо минимум `2` потока
    10.  Время ожидания обработки перед выключением сервера в случае [штатного завершения](container.md#_24)
    11.  Максимальное время жизни потока обработчика запроса
    12.  Максимальное время ожидания чтения данные из сокета/соединения
    13.  Максимальное время ожидания записи данных в сокет/соединение
    14.  Отсылать ли сообщения `keep-alive` во время жизни сокета/соединения TCP
    15.  Включает поддержку виртуальных потоков для обработки запросов (вместо `blockingThreads`), требует Java 21+
    16.  Включает логгирование модуля (по умолчанию `false`)
    17.  Включает логгирование стэка вызовов в случае исключения
    18.  Маска которая используется для скрытия указанных заголовков и параметров запроса/ответа
    19.  Список параметров запроса которые следует скрывать
    20.  Список заголовков запроса/ответа которые следует скрывать
...
Фреймворк поддерживает запуск дополнительного HTTP-сервера на отдельном порту для внутреннего API — эндпоинтов,
предназначенных для межсервисного взаимодействия во внутренней сети,
отделённых от публичных эндпоинтов в целях безопасности.

Для регистрации контроллера на внутреннем API сервере используется аннотация `@InternalApi`:
...
===! ":fontawesome-brands-java: `Java`"
    ...
    1. Указывает что контроллер будет зарегистрирован на внутреннем HTTP-сервере
...
Внутренний API сервер поддерживает телеметрию (логгирование, метрики, трассировку) так как обрабатывает бизнес-трафик.
Перехватчики (`HttpServerInterceptor`) с аннотацией `@InternalApi` будут применяться только к внутреннему серверу.
