...
- Добавлены больше конвертеров для `Json` и `StringConverter` по умолчанию

Исправлено:

- Исправлено именование полей OpenAPI `Enum` и именование полей при транслитерации
- Исправлена опция OpenAPI `filterWithModels` для работы со сложными иерархиями и рекурсией
- Исправлена поддержка свойств Kotlin в интерфейсах `@ConfigSource`
- Исправлена поддержка приватных функций и свойств в реализациях `PromisedProxy`
- Исправлена генерация `JsonReader` для запечатанных интерфейсов
