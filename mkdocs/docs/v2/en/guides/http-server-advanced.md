...
=== ":simple-kotlin: `Kotlin`"
    ...
    If you want to check your progress along the way, use the finished working example: [Kora Kotlin HTTP Server Advanced App](https://github.com/kora-projects/kora-examples/tree/master/guides/kotlin/kora-kotlin-guide-http-server-advanced-app).
...
Because request handling is synchronous, an interceptor is an ordinary method call around the rest of the chain: it calls `chain.process(request)`, gets an `HttpServerResponse` back, and can wrap the
call in `try/catch/finally`. There is no callback, no `CompletionStage`, and no `suspend` function in the contract.
...
That is what `HttpServerRequestMapper<T>` is for. It lets you derive one typed parameter from the raw HTTP request. The interface lives in `io.koraframework.http.server.common.request` and has a
single synchronous method, `T apply(HttpServerRequest request)`.

A mapper referenced from `@Mapping` is **not** created by the generated controller module — it is requested from the dependency graph. That is why the mapper class carries `@Component` even though it
has no constructor dependencies of its own.

Create the `RequestContextMapper`:

===! ":fontawesome-brands-java: `Java`"

    Add these nested types inside `UserController.java`, together with the imports for `org.jspecify.annotations.Nullable`, `io.koraframework.http.common.cookie.Cookie`,
    `io.koraframework.http.server.common.request.HttpServerRequest` and `io.koraframework.http.server.common.request.HttpServerRequestMapper`:
...
=== ":simple-kotlin: `Kotlin`"
    ...
    Add the same idea in `UserController.kt`, together with the imports for `io.koraframework.http.server.common.request.HttpServerRequest` and
...
- `@Mapping(...)` from `io.koraframework.common.annotation` tells Kora to use that mapper for one specific parameter
...
An exception thrown by a mapper is turned into a `400` response unless the exception is itself an `HttpServerResponse`, so a mapper is also a convenient place to reject a malformed request with an
exact status code.
...
Both `FormUrlEncoded` and `FormMultipart` live in `io.koraframework.http.common.form`, and Kora already provides the request mappers for them, so a controller method only has to declare the parameter.
...
The interceptor contract is one synchronous method:

- `intercept(request, chain)` receives the incoming request and the rest of the chain
- `chain.process(request)` runs the remaining interceptors and the controller method and returns the response
- returning without calling `chain.process(...)` short-circuits the route
- an exception raised deeper in the chain simply propagates, so `try/finally` is enough to always log a line

The initial `statusCode = 500` is what gets logged when the chain throws: `chain.process(request)` never returns a value in that case, and the `finally` block still runs.
...
`@InterceptWith` comes from `io.koraframework.http.common.annotation`. It is repeatable and can be placed on the controller class or on a single route method; interceptors declared on the class run
before those declared on the method. Like `@Mapping`, it references a class that Kora resolves from the graph, so `LoggingInterceptor` must be a `@Component`.
...
Kora collects global interceptors by the `@Tag(HttpServer.class)` tag: `HttpServerModule` declares the router dependency as `@Tag(HttpServer.class) All<HttpServerInterceptor> interceptors`. That tag,
and only that tag, is what makes this interceptor global rather than controller-local. `HttpServer` is imported from `io.koraframework.http.server.common`.

Because handling is synchronous, the handler is a plain `try/catch` around `chain.process(request)`. The interceptor depends on `JsonWriter<ErrorResponse>`, so it can always serialize the same typed
error body instead of building ad-hoc strings by hand. That constructor dependency is also the reason this class must be a graph component.
...
`JsonWriter.toByteArray(...)` declares no checked exception, so the handler needs no extra `IOException` branch around serialization.

???+ warning "The tag is what makes it global"

    Only `@Tag(HttpServer.class)` / `@Tag(HttpServer::class)` registers a global interceptor for the public server. Any other tag still compiles, and the interceptor is simply never invoked, so the
    error contract silently disappears. To intercept every request of the **system** server instead, use the `@SystemApi` tag. When several global interceptors exist, they are applied in a
    deterministic order sorted by the interceptor class simple name.
...
The global interceptor also wraps the `404` and `405` responses produced by the router itself, and parameter parsing failures, which Kora answers with `400` before the controller method runs. That is
what makes it a real error contract instead of a per-controller convenience.
...
This is not meant to be enterprise-grade authentication. It is a lightweight teaching example that shows how Kora interceptors and configuration can work together for authorization-like checks. For
`Principal`-based authorization built on `HttpServerPrincipalExtractor`, see [HTTP Server authorization](../documentation/http-server.md#authorization).
...
Header names are matched case-insensitively by `HttpHeaders`, so `"authorization"` also matches an `Authorization:` header sent by a client.
...
===! ":material-code-json: `Hocon`"
    ...
    1.  Local default used when no environment variable is set.
    2.  Optional override from `HTTP_ADVANCED_API_KEY`. In HOCON a default is expressed by assigning the key twice: the second assignment is skipped when the variable is absent.
...
=== ":simple-yaml: `YAML`"
    ...
    1.  Reads `HTTP_ADVANCED_API_KEY` and falls back to the shown default. Do not write `${?VAR:default}`: with `?` the whole `VAR:default` text is treated as the reference name and the key resolves to nothing.
...
## Blocking and Parallel Work { #blocking-parallel }

Kora HTTP handlers are synchronous, and that is a deliberate design decision rather than a limitation. Undertow dispatches every request onto a virtual thread before the generated handler calls your
controller method, so blocking IO inside a route is normal: a JDBC query, an HTTP call to another service, or reading a file does not pin a platform thread.

The processors enforce that contract at compile time:

===! ":fontawesome-brands-java: `Java`"

    A route that returns `CompletionStage<T>`, `Future<T>`, or a reactive `Publisher<T>` compiles, but the annotation processor reports a warning and the value is treated as an ordinary result that
    needs its own response mapper:

    ```text
    Method return type is CompletionStage<T> which is unsupported and has no meaning
    ```

    Return the value itself instead. When one request genuinely needs two independent calls at the same time, fan out inside the handler and join before returning:
    ...
    1.  One short-lived executor per request; `close()` waits for the submitted tasks.
    2.  Both calls start immediately and run on their own virtual threads.
    3.  The handler blocks here and returns a plain value, so the route contract stays synchronous.

=== ":simple-kotlin: `Kotlin`"

    A `suspend` route is rejected outright by the symbol processor:
...
The same rule applies to interceptors and to `HttpServerRequestMapper` / `HttpServerResponseMapper` implementations: all of them return their result directly.

The full compiler message for `suspend` also suggests structured concurrency with `StructuredTaskScope` for this fan-out, which is a preview API and needs `--enable-preview` on the JDK 25 toolchain.
The executor above requires no preview flag, which is why the guide uses it.
...
- `HttpServerRequestHandlerImpl.of(...)` registers the route method and path, and the generated method name is derived from them
- `HttpServerRequestMapper<FormUrlEncoded>` reads the request body and is injected from the graph
- `DataApiAuthInterceptor` wraps the route, and `chain.process(request)` is the nested lambda
- `HttpServerResponseMapper<String>` turns the return value into an HTTP response
- a mapper failure is converted into `400` unless the exception is already an `HttpServerResponse`

Notice what is *not* there: no executor hop, no `CompletableFuture`, no callback. The handler runs top to bottom on the virtual thread Undertow assigned to the request.
...
`HttpServerRequestHandlerImpl` lives in `io.koraframework.http.server.common.request` and offers one factory per HTTP method (`get`, `post`, `put`, `delete`, ...) plus a generic `of(method, route,
handler)`. The handler is a `HandlerFunction`: it takes the `HttpServerRequest` and returns an `HttpServerResponse`.
...
- the lambda receives the `HttpServerRequest` and returns an `HttpServerResponse`
- the handler reads the `Authorization` header manually
- because the method is declared on the `@KoraApp` interface, its result becomes a graph component, and the router picks it up through `All<HttpServerRequestHandler>`
- global interceptors such as `ExceptionHandler` still wrap this route, because they are applied by the router and not by the generated controller module

After compilation, the generated application graph wires this handler as another node. Component numbers depend on how many components the included modules contribute, so they will differ in your
build:
...
If the `Authorization` header is missing or wrong, the route should return `403` with the shared `ErrorResponse` body. The restricted form name produces the same shape with `400`:
...
- Use a global interceptor only for behavior that truly should affect the whole HTTP server, and always tag it with `@Tag(HttpServer.class)`.
- Cover a global interceptor with a test: a wrong tag compiles cleanly and silently disables it.
...
- global interceptors with `@Tag(HttpServer.class)`
...
- why controller methods, interceptors, and mappers are all synchronous, and how to still run work in parallel
...
**Compilation fails with `No component found for dependency` naming the mapper or interceptor:**

- A class referenced from `@Mapping` or `@InterceptWith` is resolved from the graph, never constructed by the generated module. Add `@Component` to it.
...
- Check `@Tag(HttpServer.class)` on the interceptor. Any other tag compiles but is never collected by the router.
...
**The YAML API key resolves to nothing:**

- `${?VAR:default}` is not a valid combination: with `?` the whole `VAR:default` text becomes the reference name. Use `${VAR:default}`.

**Kotlin build fails with `Suspend methods are not supported by the HTTP server controller generator`:**

- Remove `suspend` from the controller method and return the value directly.
